...
export default function MainNav() {
  ...
  return (
    <>
          ...
          <div className="flex flex-nowrap">
            ...
            <div className="flex flex-col items-center justify-center border-r border-[--grey-border]">
              <button
                ...
                className="flex flex-col items-center justify-center hover:bg-[--lightgrey-hover] lg:min-h-[75px] min-h-[50px] lg:min-w-[75px] min-w-[50px] transition-all duration-400"
              >
            ...
            {menuitems.map((item, index) => (
              ...
              >
                <Link
                  href={item.link}
                  className="flex flex-col items-center justify-center hover:bg-[--lightgrey-hover] min-h-[75px] min-w-[75px] px-4 transition-all duration-400"
                >
                  {item.name}
                </Link>
              </div>
      ...
      <nav
        className={`${
          isSideMenuOpen ? "opacity-100 w-[100vw]" : "opacity-0 w-[0vh]"
        } fixed left-0 top-0 h-full backdrop-brightness-50 z-50 transition-all`}
      >
        <div
          ref={sideMenuRef}
          className={`${
            isSideMenuOpen ? "fixed left-0" : "fixed -left-80"
          } flex flex-col w-80 h-full bg-[--lightgrey] transition-all delay-200 p-8`}
        >
          <AiOutlineClose
            onClick={toggleSideMenu}
            className="self-end min-w-8 min-h-8 cursor-pointer hover:bg-white"
          />
          {/*<input 
            type="text"
        ...
        />*/}
          ...
          {menuitems.map((item, index) => (
            ...
            >
              <Link
                href={item.link}
                className="flex flex-col items-center justify-center hover:bg-[--lightgrey-hover] min-h-[75px]  w-full transition-all duration-400"
              >
                {item.name}
              </Link>
            </div>
